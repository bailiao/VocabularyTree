...
void imageRetriver::buildDataBase(char* directoryPath) {
	...
	DirectoryList("E:\\Donald Duck\\MyClass5\\SRTP\\Pictures", databaseImagePath, ".jpg");
	double** trainFeatures = NULL;
	int nFeatures = getTrainFeatures(trainFeatures, databaseImagePath);							// 只有这里需要替换掉SIFT			
...
int imageRetriver::getTrainFeatures(double** trainFeatures, vector<string> imagePaths) {		//这个函数内部需要修改，替换掉原有的特征提取函数，仍然返回特征的总数
